...
version, or multiple. It may be desirable to deploy a version without routing to it at all, for instance to do shadowing from part 3. 
...
`echo` and `foxtrot` installed from before, just run `kubectl delete ns echo foxtrot`. Just like in 
part 3, we'll start by deploying our generic virtual service for our `example` API group. Then, as we install new services
with our helm chart, these routes will automatically be picked up. 
...
First, we'll need to introduce a lot more values to our chart in order to start supporting more workflows, and more 
types of services. As an example, we skipped adding support for the shadowing feature we looked at in part 3. There 
are potentially many other things we'd need to customize, such as the deployment image, resource limits, support 
multiple routes, alternative paths, and so forth. Hopefully, at this point you are convinced that we could tackle those
as extensions to our work so far. 
